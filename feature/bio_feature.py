...
from feature.statistics import Statistics
from utils.time_converter import convert_time, convert_int
from utils.numeric import isfloat, isint

class BioFeature(object):
    
    def __init__(self, filepath):
        self.filepath = filepath + 'biospecimen/'
    ...
    def load_feature(self, fname=None, featname=None):
        if fname == 'CSF':
            self.read_feature('Biospecimen_Analysis_Results.csv', featname, date_key='RUNDATE')
        elif fname == 'Blood':
...
#            self.read_feature('Skin_Biopsy_Eligibility.csv')
    ...
    def get_feature_name(self, filename):
        ...
        if filename == 'Biospecimen_Analysis_Results.csv':
